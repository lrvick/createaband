...
?>
   <table width=100%>
   <tr>
    <td class='lined title'>Admin: Music Forums Manager</td>
   ...
   <?
	  ...
	  if($page=='')	$page=1;
	  $start=($page-1)*20;
      $sql_query="select * from music_forums limit $start,20";
	  $p_sql="select f_id from music_forums";
	  $p_url="admin.php?mode=m_forums_manager&adsess=$adsess";
      ...
      <input type=hidden name='mode' value='m_forums_manager'>
      ...
      <tr><td><strong>Select</strong></td><td><strong>Posts</strong></td><td><strong>Forum Creator</strong></td><td><strong>Type</strong></td><td><strong>Category</strong></td>";
         ...
         while($fru=mysql_fetch_object($res)){
           $sql_query="select name from music_f_categories where f_cat_id='$fru->f_c_id'";
   ...
   ?>
   &nbsp<input value='Manage Categories' type=button onclick="window.location='admin.php?mode=m_forums_manager&act=cat&adsess=<? echo $adsess; ?>'">
   </table></td></table>
   <?
   show_footer();
}//function
...
global $base_path;
   $sql_query="delete from music_forums where f_id='$fid;'";
...
?>
   <table width=100%>
   <tr>
    <td class='lined title'>Admin: Music Forum Categories Manager</td>
   ...
   <table width=90% align="center" cellpadding=10 cellspacing=1 class='body'>
        <form action='admin.php' method=post>
         ...
         <input type='hidden' name='adsess' value='<? echo $adsess; ?>'>
         <?php
           $sql_query="select * from music_f_categories";
           $res=sql_execute($sql_query,'res');
...
$sql_query="insert into music_f_categories(name,descr) values('".addslashes($new_cat)."','".addslashes($new_cat_desc)."')";
sql_execute($sql_query,'');
...
foreach($f_cat_id as $id){
   $sql_query="delete from music_f_categories where f_cat_id='$id'";
   sql_execute($sql_query,'');
   $sql_query="select f_id from music_forums where f_c_id='$id'";
   $res=sql_execute($sql_query,'res');
   $forums=array();
   while($fru=mysql_fetch_object($res)){
