...
?>
   <table width=100%>
   <tr><td class='lined title'>Admin: Music Listings Manager</td>
   ...
   <table class='body' width=100% cellpadding=10 cellspacing=1>
        ...
        $start=($page-1)*20;
          $sql_query="select * from music_listings limit $start,20";
		  $p_sql="select lst_id from music_listings";
		  $p_url="admin.php?mode=m_listings_manager&adsess=$adsess";
          ...
          echo "<form action='admin.php' method=post>
           <input type=hidden name='mode' value='m_listings_manager'>
           ...
           <tr><td><strong>Select</strong></td><td><strong>Title</strong></td><td><strong>Listing</strong></td><td><strong>Lister</strong></td><td><strong>Category</strong></td>";
             ...
             while($lst=mysql_fetch_object($res)){
                 ...
                 <td><span class='action'><a href='admin.php?mode=m_listings_manager&act=edi&adsess=$adsess&lst_id=$lst->lst_id'>$lst->title</a></span></td>
        ...
        ?>
        &nbsp<input type=button value='Manage Categories' onclick="window.location='admin.php?mode=m_listings_manager&act=cat&adsess=<? echo $adsess; ?>'"></td>
   </table></form>
   </td>
   </table>
...
foreach($lst_id as $lid){
    $sql_query="delete from music_listings where lst_id='$lid'";
...
?>
  <table width=100% class='body'>
   <tr><td class='lined title'>Admin: Manage Music Listing Categories</td>
   ...
   <table class='body' width=100%>
         ...
         <?
            for($i=0;$i<$mid;$i++){
            $sql_query="select * from music_categories where cat_id='$cats[$i]'";
...
$sql_query="select max(cat_id) as max from music_categories";
...
$sql_query="insert into music_categories(cat_id,name) values('$new_num','$name')";
sql_execute($sql_query,'');
...
$sql_query="select max(sub_cat_id) as max from music_sub_categories where cat_id='$cat_id'";
$num=sql_execute($sql_query,'num');
if($num==0){
  $newnum=$cat_id+1;
}//if
...
$sql_query="insert into music_sub_categories(cat_id,sub_cat_id,name) values('$cat_id','$newnum','$name')";
sql_execute($sql_query,'');
...
if($sub_cat_id!=''){
  foreach($sub_cat_id as $sid){
       $sql_query="delete from music_sub_categories where sub_cat_id='$sid'";
       sql_execute($sql_query,'');
       $sql_query="delete from music_listings where sub_cat_id='$sid'";
       sql_execute($sql_query,'');
  }//foreach
}//if
...
if($cat_id!=''){
  foreach($cat_id as $cid){
      $sql_query="delete from music_categories where cat_id='$cid'";
      sql_execute($sql_query,'');
      $sql_query="delete from music_sub_categories where cat_id='$cid'";
      sql_execute($sql_query,'');
      $sql_query="delete from music_listings where cat_id='$cid'";
      sql_execute($sql_query,'');
  }//foreach
}//if
